...
rule all:
    input:
        expand("data/{sample}/{sample}_R{Num}_unmap_filter_paired_fastqc.html", sample=config["samples"], Num=["1","2"]),
...
rule qc_before_filtered:
    ...
    params:
        FQ1 = lambda wildcards: config["samples"][wildcards.sample]["R1"],
        FQ2 = lambda wildcards: config["samples"][wildcards.sample]["R2"],
        threads=8,
    container:
        "proseq.sif"
    shell:
...
rule remove_additional_nucleotide:
    ...
    shell:
        """
        seqtk trimfq -b 1 qc/{wildcards.sample}/{wildcards.sample}_R1_val_1.fq.gz > qc/{wildcards.sample}/{wildcards.sample}_R1_result_trimmed.fastq
        seqtk trimfq -b 1 qc/{wildcards.sample}/{wildcards.sample}_R2_val_2.fq.gz > qc/{wildcards.sample}/{wildcards.sample}_R2_result_trimmed.fastq
        """
...
rule prealignments:
    ...
    shell:
        """
        bowtie2 -p 16 -k 1 -D 20 -R 3 -N 1 -L 20 -i S,1,0.50 -x {params.BOWTIE2_RDNAIDX} --rg-id {wildcards.sample} -U qc/{wildcards.sample}/{wildcards.sample}_R1_result_trimmed.fastq --un qc/{wildcards.sample}/{wildcards.sample}_R1_unmap.fastq | \
        samtools view -bS - -@ 1 | \
        samtools sort - -@ 1 -o data/prealignments_{wildcards.sample}.bam
        """

rule filter_paired_fq:
    input:
        NON_RRNA_FQ1="qc/{sample}/{sample}_R1_unmap.fastq",
    output:
        NON_RRNA_filter_paired_FQ1=temp("qc/{sample}/{sample}_R1_unmap_filter_paired.fastq"),
        NON_RRNA_filter_paired_FQ2=temp("qc/{sample}/{sample}_R2_unmap_filter_paired.fastq"),
    params:
        BOWTIE2_RDNAIDX = config["BOWTIE2_RDNAIDX"],
        remove_additional_nuc_FQ1="qc/{sample}/{sample}_R1_result_trimmed.fastq",
        remove_additional_nuc_FQ2="qc/{sample}/{sample}_R2_result_trimmed.fastq",
    ...
    shell:
        """
        perl scripts/filter_paired_fq.pl \
        {input.NON_RRNA_FQ1} \
        {params.remove_additional_nuc_FQ1} \
        {params.remove_additional_nuc_FQ2} \
        qc/{wildcards.sample}/{wildcards.sample}_R1_unmap_filter_paired.fastq \
        qc/{wildcards.sample}/{wildcards.sample}_R2_unmap_filter_paired.fastq
        """

rule qc_after_filtered:
    input:
        NON_RRNA_filter_paired_FQ1="qc/{sample}/{sample}_R1_unmap_filter_paired.fastq",
        NON_RRNA_filter_paired_FQ2="qc/{sample}/{sample}_R2_unmap_filter_paired.fastq",
    output:
        html_R1="data/{sample}/{sample}_R1_unmap_filter_paired_fastqc.html",
        html_R2="data/{sample}/{sample}_R2_unmap_filter_paired_fastqc.html",
    threads: 
        8
    container:
        "proseq.sif"
    shell:
        """
        fastqc -t {threads} qc/{wildcards.sample}/{wildcards.sample}_R1_unmap_filter_paired.fastq qc/{wildcards.sample}/{wildcards.sample}_R2_unmap_filter_paired.fastq -o data/{wildcards.sample}
        """
...
rule primary_single_alignments:
    ...
    shell:
        """ 
        mkdir stat/
        bowtie2 -p {threads} --very-sensitive  -X 2000 -q --end-to-end --un-conc qc/{wildcards.sample}/{wildcards.sample}_hgRemovedTemp.fastq --rg-id {wildcards.sample} -x {params.BOWTIE2IDX} --rf -1 qc/{wildcards.sample}/{wildcards.sample}_R1_unmap_filter_paired.fastq -2 qc/{wildcards.sample}/{wildcards.sample}_R2_unmap_filter_paired.fastq -S {output.temp_sam} 2>&1 | \
...
rule Collapsing:
    ...
    shell:
        """
        awk \'$2 != 4 {{print}}\' {input.temp_sam} | sed \'/XS:/d\' | samtools view -bS - -@ 1 | samtools sort - -@ 1 -o {output.temp_BAM}
        """
...
rule index_PE1_BAM:
    input:
        sort_PE1_BAM="data/{sample}_PE1.bam",
    output:
        sort_PE1_BAM_index="data/{sample}_PE1.bam.bai",
    container:
        "proseq.sif"
    shell:
        """
        samtools index {input.sort_PE1_BAM}
        """
...
rule chr_order:
    ...
    shell:
        """
        samtools view -H {params.sort_PE1_BAM} | grep \'SN:\' | awk -F\':\' \'{{print $2,$3}}\' | awk -F\' \' -v OFS=\'\\t\' \'{{print $1,$3}}\' > {output.chr_order_txt}
        """
...
rule TssEnrichment:
    ...
    shell:
        """
        python scripts/pyTssEnrichment.py -a {params.sort_PE1_BAM} -b {params.plus_TSS} -p ends -c {params.threads} -z -v -s 6 -o stat/{wildcards.sample}_minus_TssEnrichment.txt
        python scripts/pyTssEnrichment.py -a {params.sort_PE1_BAM} -b {params.minus_TSS} -p ends -c {params.threads} -z -v -s 6 -o stat/{wildcards.sample}_plus_TssEnrichment.txt 
        """
...
rule Generate_BigWig:
    ...
    shell:
        """
        bamCoverage -b {params.sort_PE1_BAM} -o data/{wildcards.sample}_plus.bw --filterRNAstrand forward --binSize 1 --ignoreDuplicates --minMappingQuality 30 --normalizeUsing CPM -p {params.threads} 
        bamCoverage -b {params.sort_PE1_BAM} -o data/{wildcards.sample}_minus.bw --filterRNAstrand reverse --binSize 1 --ignoreDuplicates --minMappingQuality 30 --normalizeUsing CPM -p {params.threads} 
        """
...
rule converted_bed:
    ...
    shell:
        """
        bedtools bamtobed -i {params.sort_PE1_BAM} > {output.convert_bed}
        """
...
rule exons_introns_coverage:
    ...
    shell:
        """
        bedtools coverage -sorted -counts -s -a bed/{wildcards.sample}_exons_sort.bed -b {params.sort_PE1_BAM} -g {params.chr_order_txt} > bed/{wildcards.sample}_exons_coverage.bed
        bedtools coverage -sorted -counts -s -a bed/{wildcards.sample}_introns_sort.bed -b {params.sort_PE1_BAM} -g {params.chr_order_txt} > bed/{wildcards.sample}_introns_coverage.bed
        """
...
rule exons_introns_rpkm:
    ...
    shell:
        """
        ar=$(samtools view -F 4 -c {params.sort_PE1_BAM})
